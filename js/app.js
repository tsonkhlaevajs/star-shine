 ...
 */
    ...
    (() => {
        ...
        if (window.innerWidth <= 767.98) if (document.querySelector(".product-tabs__tabs")) {
            let tabsProduct = document.querySelector(".product-tabs__tabs");
            tabsProduct.removeAttribute("data-tabs");
            tabsProduct.removeAttribute("data-tabs-titles");
            tabsProduct.removeAttribute("data-tabs-body");
        }
        const script_showMore = document.querySelector(".show-more");
        const productsLength = document.querySelectorAll(".news-item").length;
        let items = 2;
        if (document.querySelector(".show-more")) script_showMore.addEventListener("click", (() => {
            items += 2;
            const array = Array.from(document.querySelector(".news-articles").children);
            const visItems = array.slice(0, items);
            visItems.forEach((el => el.classList.add("is-visible")));
            if (visItems.length === productsLength) script_showMore.style.display = "none";
        }));
        if (window.innerWidth <= 767.98) if (document.querySelector(".show-more")) script_showMore.addEventListener("click", (() => {
            items += 13;
            const array = Array.from(document.querySelector(".news-articles").children);
            const visItems = array.slice(0, items);
            visItems.forEach((el => el.classList.add("is-visible")));
